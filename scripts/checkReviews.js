...
// DEBUG_NOTIFY=true のときだけ、notified_reviewsを無視して強制通知（動作テスト用）
...
async function main() {
            ...
            if (DEBUG_NOTIFY) console.log('[DEBUG] デバッグモード: 強制通知テスト');
  ...
  // DEBUGモード時は最初の1店舗だけ処理
  const targets = DEBUG_NOTIFY ? uniquePlaces.slice(0, 1) : uniquePlaces;
  ...
  for (const place of targets) {
              ...
              // DEBUGモード時は最初の1件だけ強制送信（notified_reviewsを無視）
              const reviewsToProcess = DEBUG_NOTIFY ? reviews.slice(0, 1) : reviews;
