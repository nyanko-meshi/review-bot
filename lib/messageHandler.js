...
export async function handleMessage(event) {
  ...
  if (state === 'awaiting_store_selection') {
    ...
    try {
      ...
      await replyMessage(replyToken, [textMessage('店舗情報の取得に失敗しました。')]);
      return;
    }
    ...
    const { error } = await supabase.from('watched_stores').insert({
      ...
      place_name: details.name ?? selected.name,
    ...
    if (error?.code === '23505') {
      await replyMessage(replyToken, [textMessage(`「${selected.name}」はすでに登録済みです。`)]);
    } else {
      await replyMessage(replyToken, [
        textMessage(`✅ 「${details.name ?? selected.name}」を登録しました！\n新しい口コミが投稿されたらお知らせします🔔`),
      ]);
    }
    return;
  }
