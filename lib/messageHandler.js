...
import { replyMessage, textMessage, quickReply } from './line.js';
...
const MAIN_MENU = {
  ...
  quickReply: {
    items: [
      ...
      { type: 'action', action: { type: 'message', label: '➕ 登録', text: '登録' } },
      { type: 'action', action: { type: 'message', label: '🗑️ 削除', text: '削除' } },
    ]
  }
};
...
export async function handleMessage(event) {
  ...
  if (text === '削除') {
    ...
    if (!stores?.length) {
      await replyMessage(replyToken, [textMessage('削除できる店舗がありません。'), MAIN_MENU]);
      return;
    }
  ...
  if (state === 'awaiting_delete_selection') {
    ...
    await clearState(lineUserId);
    await replyMessage(replyToken, [
      textMessage(`「${store.place_name}」の監視を解除しました。`),
      MAIN_MENU,
    ]);
    return;
  }
  ...
  if (state === 'awaiting_store_name') {
    ...
    try {
      ...
      await replyMessage(replyToken, [textMessage('検索中にエラーが発生しました。もう一度お試しください。'), MAIN_MENU]);
      return;
    }

    if (!places.length) {
      await replyMessage(replyToken, [
        textMessage(`「${text}」の店舗が見つかりませんでした。別のキーワードで試してください。`),
      ]);
      return;
    }
  ...
  if (state === 'awaiting_store_selection') {
    if (text === '0' || text === 'キャンセル') {
      await clearState(lineUserId);
      await replyMessage(replyToken, [textMessage('キャンセルしました。'), MAIN_MENU]);
      return;
    }
    ...
    try {
      ...
      await replyMessage(replyToken, [textMessage('詳細取得中にエラーが発生しました。もう一度お試しください。'), MAIN_MENU]);
      return;
    }
    ...
    const { error } = await supabase.from('watched_stores').insert({
      ...
      place_url: details.url ?? null,
      last_review_count: details.reviews?.length ?? 0,
    });
    ...
    if (error) {
      ...
      await replyMessage(replyToken, [textMessage('登録中にエラーが発生しました。もう一度お試しください。'), MAIN_MENU]);
    } else {
      await replyMessage(replyToken, [
        textMessage(`✅「${selected.name}」を登録しました！\n新しい口コミが投稿されたらお知らせします。`),
        MAIN_MENU,
      ]);
    }
    return;
  ...
  // --- デフォルト：メニュー表示 ---
  await clearState(lineUserId);
  await replyMessage(replyToken, [MAIN_MENU]);
}
