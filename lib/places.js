...
export async function searchPlaces(query) {
    ...
    console.log('Places API status:', data.status, 'error_message:', data.error_message);
...
export async function getPlaceDetails(placeId) {
    ...
    const res = await fetch(url);
    const data = await res.json();
    console.log('Place Details status:', data.status, 'error_message:', data.error_message);
